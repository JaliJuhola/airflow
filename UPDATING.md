...
### Changes in writing Logs to Elasticsearch (X)
...
### Changes to the Google Cloud Storage Hook (X)
...
### Increase standard Dataproc disk sizes (X)
...
### Changes to SalesforceHook (X)

* renamed `sign_in` function to `get_conn` 

### HTTPHook verify default value changed from False to True. (X)
...
### Changes to GoogleCloudStorageHook (X)
...
### Unify default conn_id for Google Cloud Platform (X)
...
### New `dag_discovery_safe_mode` config option (X)
...
### Removed deprecated import mechanism (This .......)
...
### RedisPy dependency updated to v3 series (X)
...
### SLUGIFY_USES_TEXT_UNIDECODE or AIRFLOW_GPL_UNIDECODE no longer required (X)
...
### new `sync_parallelism` config option in celery section (X)
...
### Rename of BashTaskRunner to StandardTaskRunner (X)
...
### Modification to config file discovery(X)
...
### Changes in Google Cloud Platform related operators (X)
...
### Changes in Google Cloud Platform related hooks (X)
...
### Changed behaviour of using default value when accessing variable (X)
...
### Removal of `airflow_home` config setting (X ....)
...
### Change of two methods signatures in `GCPTransferServiceHook` (X)
...
### Moved two classes to different modules (X)
...
### Fixed typo in --driver-class-path in SparkSubmitHook (X)
...
### DAG level Access Control for new RBAC UI (X)
...
### Modification to `ts_nodash` macro (X)
...
### Semantics of next_ds/prev_ds changed for manually triggered runs (X)
...
### Custom auth backends interface change (X)
...
### Support autodetected schemas to GoogleCloudStorageToBigQueryOperator (X)
...
### EMRHook now passes all of connection's extra to CreateJobFlow API (XXX)
...
### Replace DataProcHook.await calls to DataProcHook.wait (X)

The method name was changed to be compatible with the Python 3.7 async/await keywords

### Setting UTF-8 as default mime_charset in email utils (X)

### Add a configuration variable(default_dag_run_display_number) to control numbers of dag run for display (X)
...
### Default executor for SubDagOperator is changed to SequentialExecutor (X)

### New Webserver UI with Role-Based Access Control (X)
...
### Google cloud connection string (X)
...
### SSH Hook updates, along with new SSH Operator & SFTP Operator (X)
...
### S3Hook switched to use Boto3 (X)
